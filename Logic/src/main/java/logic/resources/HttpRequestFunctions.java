...
public class HttpRequestFunctions {
    ...
    public static void httpRequestSearch(String link1, String search, String link2, String outFile) {
        URL url;
        try {
            if (!search.isEmpty()) {
                //Criar URL com palavra de pesquisa
//                System.out.println(link + URLEncoder.encode(pesquisa,"UTF-8").replace("+", "_"));
                url = new URL("https://" + link1 + URLEncoder.encode(search, "UTF-8").replace("+", "_") + link2); //alterar replace se necessário
            } else {
                //Criar URL simples, sem palavra de pesquisa
                System.out.println(link1 + search + link2);
                url = new URL(link1 + search + link2);
            }
    ...
    public static void httpRequest1(String link, String pesquisa, String outFile) {
        URL url;
        try {
            ...
            if (!pesquisa.isEmpty()) {
                //Criar URL com palavra de pesquisa
//                System.out.println(link + URLEncoder.encode(pesquisa,"UTF-8").replace("+", "_"));
                url = new URL("https://" + link + URLEncoder.encode(pesquisa, StandardCharsets.UTF_8).replace("+", "_").replace("%2F", "/").replace("%28", "(").replace("%29", ")")); //alterar replace se necessário
            } else {
                //Criar URL simples, sem palavra de pesquisa
